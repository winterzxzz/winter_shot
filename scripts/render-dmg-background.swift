...
// Nameplate capsules where Finder draws the icon labels (same treatment as
// winter_voice's installer). Finder draws label text in black whenever the
// view has a background picture and exposes no way to recolor it — the
// mid-gray capsule is the tone that keeps black text readable while sitting
// comfortably on the dark canvas. Centered on the measured label band: the
// 12pt label under a 100pt icon at y=192 spans 258.5–266.5pt from the
// window top.
for target in targetCenters {
    let capsule = NSBezierPath(
        roundedRect: NSRect(x: target.x - 76, y: fromTop(262.5) - 15, width: 152, height: 30),
